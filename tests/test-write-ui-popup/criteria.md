...
7. To alternate the text between the two values every second, the Lua script uses either (a) a `Timer` initiated in `self:Start()` or `self:Awake()` or `self:OnEnable()`, or (b) a counter incremented in `self:Update()`.
