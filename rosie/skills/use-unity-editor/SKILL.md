...
1. Check that `Temp/Highrise/Serializer/active_scene.json` exists. If it does not, follow these steps:
   ...
   b. Ensure that the `Assets/Scripts/Serializer/` folder is in the project's `.gitignore` file, if one exists, so that the symlinks are not committed to the repository.
